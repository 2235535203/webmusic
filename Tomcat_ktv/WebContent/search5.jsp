                   ...
                   // alert(jsonlength);
                    ...
                    for(var i = 0;i<jsonlength;i++){
                        str1 += "<tr class='myclass'>" +                         
                            ...
                            "<td>"+jsonobj[i].creator_gender+"</td>" +
                            "<td>"+jsonobj[i].creator_country+"</td>" +
                            "<td>"+jsonobj[i].creator_area+"</td>" +
                            "<td>"+jsonobj[i].creator_remark+"</td>" +
                            "<td hidden='hidden'>"+jsonobj[i].creator_id+"</td>" +
                            "<td>"+"<button class='playmusic'>详情</button>"+"</td>" +
                            "</tr>";
...
$(document).ready(function(){
  ...
  $("#table_result").on('click','.playmusic',function(){
    ...
    var creator_name=currentRow.find("td:eq(0)").html(); //获得当前和第一个表格单元格TD值
    var creator_gender=currentRow.find("td:eq(1)").html(); //获得当前和第一个表格单元格TD值
    var creator_country=currentRow.find("td:eq(2)").html(); //获得当前和第一个表格单元格TD值
    var creator_area=currentRow.find("td:eq(3)").html();
    var creator_remark=currentRow.find("td:eq(4)").html();
    var creator_id=currentRow.find("td:eq(5)").html();
    ...
    var musicjson= {
    	    ...
    	    "creator_area":creator_area,
    	    "creator_remark":creator_remark,  
    	    "creator_id":creator_id, 
    	   
    	};
       var mymusicJSON = JSON.stringify(musicjson);
       sessionStorage.setItem("album_json",mymusicJSON);
      // alert(mymusicJSON);
       window.location.href="singer.jsp";
      /* $.ajax({
           //几个参数需要注意一下
               type: "POST",//方法类型，或者用GET
               dataType: "text",//预期服务器返回的数据类型，可以改成json
...
<table id = "table_result" align="center"  border="1" width="800" style="background-color:#fff">
        ...
        <th>地区&nbsp;&nbsp;</th>    
        <th>备注&nbsp;&nbsp;</th>    
        <th hidden="hidden">creator_id&nbsp;&nbsp;</th>
        <th>操作&nbsp;&nbsp;</th>
        </tr>
